...
public static class Toos
{
    ...
    public static GameObject CreateUIpanel(PanelType panelType)
    {
        ...
        if (prefab == null)
        {
            Debug.LogWarning("这个面板不纯在" + panelType.ToString());
    ...
    public static int GetWeight(List<Card>cards,CardType cardType)
    {
        ...
        if (cardType == CardType.ThreeAndOne || cardType == CardType.ThreeAndTwo)
        {
            ...
            {
                if (cards[i].CardWeight == cards[i + 1].CardWeight && cards[i].CardWeight == cards[i + 2].CardWeight)//判断三张一样的
                {

                    totalWeight += (int)cards[i].CardWeight;//将权值加到totalWeight
                    totalWeight *= 3;
                    break;
